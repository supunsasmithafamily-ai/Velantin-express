...
type Page = 'landing' | 'register' | 'chats' | 'thread' | 'live' | 'liveStage' | 'status' | 'wallet' | 'verify' | 'admin' | 'profile'
...
function LandingPage({ onLogin, onGoRegister, wsConnected, wsError }: {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    onLogin(name.trim(), email.trim(), password.trim())
  }
    ...
    <div className="ve-hero">
      <form className="ve-hero-card ve-panel" onSubmit={handleSubmit}>
        ...
        <button className="ve-btn ve-btn-primary" style={{ width: '100%', marginTop: 14 }} type="submit" disabled={!wsConnected}>
          {wsConnected ? 'Enter the stage' : 'Connecting to server…'}
        </button>
...
function Shell({ page, setPage, threadChatId, setThreadChatId, user, setUser, netState, emit, socket, setStatusMsg, onLogout }: {
  ...
  const navItems = [
    { to: 'chats' as Page, label: 'Chats' },
    { to: 'status' as Page, label: 'Status' },
    { to: 'live' as Page, label: 'Live' },
    { to: 'wallet' as Page, label: 'Wallet' },
    { to: 'verify' as Page, label: 'Verify' },
    { to: 'profile' as Page, label: 'Profile' },
  ]
  ...
  return (
    <div className="ve-shell">
      ...
      <aside className="ve-rail">
        ...
        <button className="ve-nav-btn ve-logout-btn" title="Sign out" onClick={onLogout}>
      ...
      <main style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        {page === 'live' && <LivePage netState={netState} emit={emit} user={user} setPage={setPage} setThreadChatId={setThreadChatId} />}
      ...
      <nav className="ve-mobile-tabs">
        {navItems.map(i => (
          <button key={i.to} className="ve-btn ve-btn-ghost" onClick={() => navTo(i.to)}>
            {i.label}
          </button>
        ))}
      </nav>
...
function LivePage({ netState, emit, user, setPage, setThreadChatId }: {
      ...
      <div style={{ padding: 20, display: 'grid', gap: 16 }}>
        ...
        <div className="ve-grid-2">
          {netState.lives.map(l => (
            <button key={l.id} className="ve-panel" style={{ textAlign: 'left' }} onClick={() => { setPage('liveStage'); setThreadChatId(l.id) }}>
...
function LiveStagePage({ netState, emit, user, setPage }: {
  ...
  return (
    <section className="ve-stage" style={{ gridColumn: '2 / -1' }}>
      <div className="ve-topbar">
        <div>
          <strong><span className="ve-live-dot" /> LIVE · {live.host}</strong>
          <div className="ve-muted">{live.title} · {live.viewers} watching · {status}</div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          {isHost && (
            <button className="ve-btn" onClick={() => {
              emit({ type: 'live_end', liveId: live.id })
              setPage('live')
            }}>End live</button>
          )}
          <button className="ve-btn" onClick={() => setPage('live')}>← Back</button>
        </div>
      </div>
      <div className="ve-live-frame">
        ...
        <div className="ve-live-overlay">
          <div className="ve-badge"><span className="ve-live-dot" /> {live.host}</div>
          <div>
            {comments.slice(-6).map((c, i) => (
              <div key={i} className="ve-badge" style={{ display: 'block', marginTop: 6 }}>
                <b>{c.user}</b> {c.text}
              </div>
            ))}
            {netState.giftFlash && <div className="ve-badge" style={{ marginTop: 8 }}>{netState.giftFlash}</div>}
          </div>
        </div>
      </div>
      <div style={{ padding: '0 16px 16px' }}>
        <div className="ve-gifts">
          {GIFT_CATALOG.map(g => (
            <button key={g.id} className="ve-gift" onClick={() => emit({ type: 'live_gift', liveId: live.id, giftId: g.id })}>
              <b>{g.name}</b>
              <small>{g.coins} coins</small>
            </button>
          ))}
        </div>
        <form className="ve-composer" onSubmit={e => {
          ...
          <input className="ve-field" style={{ marginTop: 0 }} value={text} onChange={e => setText(e.target.value)} placeholder="Say something" />
          <button className="ve-btn ve-btn-primary" type="submit">Send</button>
        </form>
      </div>
    </section>
...
function WalletPage({ user, setUser }: { user: AuthUser; setUser: (msg: string) => void }) {
  ...
  async function handleDailyClaim() {
    ...
    try {
      ...
      const data = await res.json()
      if (data.ok) {
        setBonusMsg(`+${data.coinsAwarded} coins! Come back tomorrow.`)
        setUser('refresh')
      } else {
        const hours = data.nextClaimAt ? Math.ceil((new Date(data.nextClaimAt).getTime() - Date.now()) / 3600000) : null
        setBonusMsg(hours ? `Already claimed — next bonus in ~${hours}h.` : (data.error || 'Failed'))
      }
    } catch {
  ...
  return (
    <section className="ve-stage" style={{ gridColumn: '2 / -1' }}>
      ...
      <div style={{ padding: 20 }} className="ve-grid-2">
        ...
        <div className="ve-panel">
          <h3 style={{ marginTop: 0 }}>Free coins</h3>
          <button className="ve-btn" style={{ marginRight: 8 }} onClick={handleDailyClaim} disabled={claimingDaily}>
            {claimingDaily ? 'Claiming…' : 'Claim daily bonus'}
          </button>
          <button className="ve-btn" onClick={handleWatchAd} disabled={watchingAd}>
            {watchingAd ? 'Watching…' : 'Watch ad for coins'}
          </button>
...
function VerifyPage({ user, setUser }: { user: AuthUser; setUser: (msg: string) => void }) {
  ...
  async function handleFileUpload(
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) { setMsg(data.error || 'Upload failed'); setUploading(null); return }
      setUrl(data.url)
    } catch {
      setMsg('Upload error')
    }
...
function ProfilePage({ user, setUser, onLogout }: { user: AuthUser; setUser: (u: AuthUser) => void; onLogout: () => void }) {
  ...
  async function handleAvatarUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const uploadRes = await authFetch('/api/uploads', { method: 'POST', body: form })
      const uploadData = await uploadRes.json()
      if (!uploadRes.ok) { setMsg(uploadData.error || 'Upload failed'); setUploading(false); return }
      ...
      setUser({ ...user, avatarUrl: uploadData.url })
...
export default function Home() {
      ...
      if (existingToken) {
        ...
        authFetch('/api/auth/me').then(r => r.ok ? r.json() : Promise.reject())
          .then((meData: any) => {
            ...
            setPage(p => p === 'landing' || p === 'register' ? 'live' : p)
          })
          .catch(() => { clearToken() })
      }
    })
  ...
  async function handleLogin(name: string, email: string, password: string) {
    ...
    try {
      ...
      // Store the signed session token for all subsequent requests
      setToken(data.token)
      ...
      // Connect to WS with the signed session token
      if (socket?.connected) {
